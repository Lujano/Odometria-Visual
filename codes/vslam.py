...
def thresholdTrans(img, thresholdy, w, h):
    imgy = img[:,1]
    # print(w)
    # print(h)
    # print("sd")

    valid = (imgy <(h-thresholdy)) &(imgy > thresholdy)
...
def thresholdRot(img, thresholdx, w, h):
    ...
    valid = (imgx <(w-thresholdx)) & (imgx > thresholdx)
    index = np.where(valid <= 0)
    imgPoints = np.delete(img, index, 0)
...
while True:
    ...
    im1KPts_means = thresholdRot(im1KPts_means, 100, w1, h1)
    #im1KPts_means = thresholdTrans(im1KPts_means, 20, w1, h1)
    ...
    #im2KPts_means2, im2KPts_status2, im2KPts_err2 = cv2.calcOpticalFlowPyrLK(im1, im2, im1KPts_means, None,
    #                                                                     winSize=(21, 21), maxLevel=3,
    #                                                                    criteria=criteria, minEigThreshold=0.001)
    # Keep only valid points
    im2KPts_status = im2KPts_status.astype(np.bool).ravel()

#    im2KPts_status2 = im2KPts_status2.astype(np.bool).ravel()
    #
