...
cv::CommandLineParser parser(argc, argv,
    ...
    string video_name= parser.get<string>("@image"); // Direccion del directorio con imagenes
    int index1_int;
   ...
   video_in.open(video_name);
    ...
    while(1){ // Loop de lectura de video
        video_in >> src;
        video_in >> src2;
        if(src2.empty()){ // Imagen vacia
          break;
        }
        Mat gray1, gray2;
...
int Odometry(Mat img_1, Mat img_2, Mat &R, Mat &t, Mat &imageOut){
  ...
  int thresholdx = 60, thresholdy = 40;
